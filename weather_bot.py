...
from weather_service import WeatherServiceException, WeatherInfo, get_weather_for_city, get_weather_for_location, get_city
...
async def get_weather_in_city(message: types.Message):
    ...
    response = get_message('weather_in_city_message') \
        .format(get_city(), weather.status, weather.temperature)+ '\n\n' + \
        get_advice(weather)
